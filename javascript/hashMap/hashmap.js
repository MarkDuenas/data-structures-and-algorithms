...
class HashMap {
  ...
  set(key, value) {
    ...
    let LL = this.map[hash].append(entry);
    return LL;
  }
  ...
  get(key) {
    ...
    let node = this.map[hashed];

    let current = node && node.head;

    while (current) {
      let [checkVal] = current.value.split(":");
...
// let nas = people.get("Nassir");
// let dom = people.get("Dominic");
// console.log(nas);
// console.log(dom);

// console.log(people.has("Rosie"));
// console.log(people.has("Jake"));
