...
const mapCurrentEvents = () => {
  const news = currentEvents.news.map( (value) => {
...
const countNumberOfElements = (arr) => {
  const elements = arr.reduce( (accumulator, value, index) => {
      accumulator++;
      return accumulator;
  }, 0);
  return elements;
...
const reversedString = (str) => {
  ...
  const string = stringArr.reduce((acc, val, idx) => {
      acc.unshift(val);
      return acc;
  }, []);
    return string.join(''); 
...
const countNumberOfChildren = (arr) => {
  const children = arr.reduce( (acc, val, idx) => {
      if(val.children) {
